...
func timeToMergeSort(tam int) time.Duration {
	...
	quickSort(numbersToBeSorted, 0, len(numbersToBeSorted)-1)
...
func quickSort(vetorASerOrdenado []int, inicio, fim int) []int {
	...
	if inicio < fim {
		...
		go func() {
			quickSort(vetorASerOrdenado, inicio, p-1)
			canalTerminou <- struct{}{}
		}()
		quickSort(vetorASerOrdenado, p+1, fim)
		<-canalTerminou
	}
